...
```bash
conda run -n V2E_zoo python image/run_i2e.py --help
```

필요 패키지는 루트 [requirements.txt](../requirements.txt)에 모두 포함되어 있고, image-only 설치용으로 [image/requirements.txt](requirements.txt)를 따로 유지합니다.

```bash
conda run -n V2E_zoo python -m pip install -r requirements.txt
```
...
```bash
conda run -n V2E_zoo python image/run_i2e.py image/I2E/assets --limit 1
```
...
```bash
conda run -n V2E_zoo python image/run_eventgan.py image/I2E/assets
```

위 명령은 디렉토리 안의 앞 2개 이미지를 pair로 사용합니다. 제대로 쓰려면 같은 장면의 연속 프레임을 직접 지정하는 것이 좋습니다.

```bash
conda run -n V2E_zoo python image/run_eventgan.py \
...
```bash
conda run -n V2E_zoo python image/run_tetci_byeongjun.py image/I2E/assets --limit 1
```

threshold sweep:

```bash
conda run -n V2E_zoo python image/run_tetci_byeongjun.py \
  image/I2E/assets \
  --limit 1 \
